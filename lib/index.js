...
export const getFileList = (folder, globs) =>
  walkSync(folder, { globs: ensureArray(globs) }).map(trimTrailingSlash);

export const fileWriter = (filename, str) => fs.writeFileSync(filename, str);
...
export default function indexr(inputFolder, ...rest) {
  ...
  const givenOptions = typeof lastArg === 'object' ? lastArg : defaultOptions;
