...
const shouldShowLog = (level) => {
  ...
  const indexLogLevel = Number(levels.indexOf(logLevel));
  const indexThisLevel = Number(levels.indexOf(level));
...
const getLogger = (level) => (message) => {
  ...
  messageHistory.push({ level, message: chalk.stripColor(message) });
};
